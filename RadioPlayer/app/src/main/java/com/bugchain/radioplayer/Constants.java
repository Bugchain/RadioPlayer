package com.bugchain.radioplayer;
/*
 * Created by BugChain on 7/26/17.
 * chain.chaiyaphat@gmail.com
 */

public class Constants {

    public static final String RADIO_KEY_FILTER = "com.bugchain.radioplayer";
    public static final String RADIO_KEY_VALUE = "key_error";

    public static final String ON_PREPARE = "on_prepare";
    public static final String ON_PLAYING = "on_playing";
    public static final String ON_ERROR = "on_error";
    public static final String ON_BUFFERING = "on_buffering";
    public static final String ON_STOP = "on_stop";


}
